    ...
    <el-container style="width:70%; height: 100%; margin-left: 15%">
        <el-aside width="55%">
            ...
            <div>
                <div style="float:left; margin-left: 30%">
                    <router-link :to="{ path: '/categoryitem/'+ goods.goodsCategoryId}">
                ...
                <div style="float:left; margin-left: 10%">
                    <div v-if="isCollect" style="color:RGB(230,162,60)">
                        取消收藏
                        <el-button :disabled="disabled" type="warning" icon="el-icon-star-on" @click="collect"
                                   circle></el-button>
                    </div>
                    <div v-if="!isCollect" style="color:RGB(230,162,60)">
                        收藏商品
                        <el-button :disabled="disabled" type="warning" icon="el-icon-star-off" @click="collect"
                                   circle></el-button>
                    </div>
                </div>
            </div>
        ...
        <el-main style="height: 80%; margin-top: 5%; text-align: left;">
            ...
            <div>
                ...
                <div v-for="(speckey,index) in speckeys " :key="index" style="margin-top:5%">
                    <el-radio-group v-model="choosespec[index]" @change="changespec">
                        <el-radio-button v-for="(specvalue,index2) in Object.values(specmap)[index]" :key="index2"
                                         :label="Object.values(specmap)[index][index2].id">
                            {{ Object.values(specmap)[index][index2].valueName }}
                        </el-radio-button>
                    </el-radio-group>
                </div>
                <div>
                    <el-input-number v-model="num" :min="1" :max="10" label="请选择商品数量"
                                     style="float: left; margin-top: 5%"></el-input-number>
            ...
            <div style="margin-top: 30px">
                <el-button :disabled="disabled" type="primary" icon="el-icon-circle-plus" @click="addcart"
                           style="background-color:RGB(245,154,35); border-color: RGB(245,154,35)">加入购物车
                </el-button>
                <el-button :disabled="disabled" type="primary" icon="el-icon-goods" @click="FormVisible = true"
                           style="background-color:RGB(245,154,35);border-color: RGB(245,154,35)">现在购买
                </el-button>
            </div>
            <div v-if="FormVisible" style="margin-top: 5%;">
                ...
                <router-link :to="{ path: '/payfororder'}">
                    <el-button type="primary" @click="buy"
                               style="float:left;background-color:RGB(245,154,35);border-color: RGB(245,154,35)">确定购买
                    </el-button>
                </router-link>
            </div>
        </el-main>
...
export default {
    ...
    methods: {
        changespec() {
            axios.post("http://localhost:8083/goods/stockPrice", Qs.stringify({
                good_id: this.goodsId,
                specs: this.choosespec
            }, {indices: false}))
                .then(res => {
        ...
        buy() {
            axios.post("http://localhost:8083/order/creatOrder", Qs.stringify({
                goods: this.spec_id,
                num: this.num,
                singlePrice: this.singelprice,
                u_id: this.userId,
                status: 0,
                price: this.singelprice * this.num,
                address: this.address
            }))
        },
        addcart() {
            if (this.spec_id != null) {
                axios.post("http://localhost:8083/cartitem/addcartitem", Qs.stringify({
                    userId: this.userId,
                    goodsId: this.spec_id,
                    goodsNum: this.num
                }))
                    .then(res => {
        ...
        collect() {
            if (this.isCollect) {
                ...
                axios.post("http://localhost:8083/collect/addtocollect", Qs.stringify({
                    userId: this.userId,
                    goodsId: this.spec_id
                }))
                    .then(res => {
